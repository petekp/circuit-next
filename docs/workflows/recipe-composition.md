...
`src/workflows/<flow>/artifacts.ts` as concrete Zod types with
...
`workflow.brief@v1` block contract.

**Contract aliases bridge the two.** Each schematic declares a
`contract_aliases` array, e.g.
`{ generic: 'workflow.brief@v1', actual: 'fix.brief@v1' }`. The schematic
...
1. Define per-flow schemas in `src/workflows/<wf>/artifacts.ts`.
