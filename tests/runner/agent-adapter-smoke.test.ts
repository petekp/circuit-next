...
// subprocess-tool-surface level (the subprocess's init event
// enumerates the available tools).

const AGENT_SMOKE = process.env.AGENT_SMOKE === '1';

describe('agent adapter smoke (capability boundary)', () => {
  ...
  (AGENT_SMOKE ? it : it.skip)(
    ...
    async () => {
      ...
      // dispatch.request.request_payload_hash).
      expect(result.request_payload).toBe(prompt);

      // receipt_id is the Claude-side session UUID. The exact format is
      // adapter-owned (receipt_id is z.string().min(1)); we assert
      // non-empty + non-whitespace per DispatchReceiptEvent's
      // refinement.
      ...
      // CLI version is captured from the subprocess init event —
