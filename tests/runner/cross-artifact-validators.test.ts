...
describe('runCrossArtifactValidator — sweep.batch ⊆ sweep.queue.to_execute', () => {
  let runRoot: string;
  let workflow: Workflow;
  let queueRel: string;

  beforeEach(() => {
    runRoot = mkdtempSync(join(tmpdir(), 'cross-artifact-test-'));
  ...
  function writeQueue(queue: unknown): void {
    const queueAbs = join(runRoot, queueRel);
