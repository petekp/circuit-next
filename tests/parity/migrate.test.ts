...
async function buildChildRunner(options: CompiledFlowRunOptions): Promise<GraphRunResult> {
...
describe('migrate runtime parity', () => {
  it.each([
    { label: 'default', entryModeName: undefined },
    { label: 'autonomous', entryModeName: 'autonomous' },
  ])(
    'runs the generated migrate $label flow through the runtime sub-run path',
    async ({ entryModeName }) => {
      ...
      await withTempRun(async (runDir) => {
        const result = await runSimpleCompiledFlow({
          ...
          goal: 'Migrate a dependency with runtime',
