// Property tests for the workflow router's collision / tie-breaking
...
// matching package has `skipOnPlanningArtifact` set AND the task text
// also matches the planning-artifact regex, the match is suppressed
...
//
// The example-based tests in tests/contracts/workflow-router.test.ts
...
//   - which routables enable planning-artifact suppression
...
function makeRoutable(spec: FixturePackage): RoutablePackage {
  ...
  const routing: WorkflowRoutingMetadata = {
    order: spec.order,
    signals: [
      {
        label: signalLabel,
    ...
    skipOnPlanningArtifact: spec.skipOnPlanningArtifact,
    reasonForMatch: () => `${spec.id} signal matched`,
    isDefault: spec.isDefault,
    ...(spec.isDefault ? { defaultReason: `${spec.id} is the conservative default` } : {}),
  };
  // Minimal WorkflowPackage stub. The classifier only reads pkg.id;
...
// Match the planning-artifact regex anchored in router.ts. Any of
...
describe('workflow router — first-match-wins by order, with planning-artifact suppression', () => {
  it('returns the lowest-order non-default routable whose signal matches and is not suppressed; falls back to default otherwise', () => {
    ...
    for (let i = 0; i < 300; i++) {
      ...
      const fixturePackages: FixturePackage[] = tokens.map((token, idx) => {
        ...
        return {
          ...
          skipOnPlanningArtifact: !isDefault && nextBool(rng),
          isDefault,
        };
      });
      ...
      for (const r of sorted) {
        ...
        const matched = r.routing.signals.some((s: WorkflowSignal) => s.pattern.test(taskText));
        if (!matched) continue;
        const suppressed = r.routing.skipOnPlanningArtifact === true && includePlanning;
      ...
      expect(
        ...
        `case ${i} bucket=${predictionBucket}: predicted ${predicted.pkg.id}, got ${decision.workflowName}; taskText='${taskText}'`,
    ...
    // skipOnPlanningArtifact AND the task text includes a planning
  ...
  it('is total: every (taskText, routables, defaultPackage) tuple yields some decision', () => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      const decision = classifyTaskAgainstRoutables(taskText, sorted, defaultPackage);
      expect(decision.workflowName).toMatch(/^(review|build|explore)$/);
