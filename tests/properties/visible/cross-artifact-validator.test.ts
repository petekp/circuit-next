// Property test for the cross-artifact validator binding sweep.batch
// items to sweep.queue.to_execute (FU-T12 area: cross-artifact authority
...
// src/workflows/sweep/cross-artifact-validators.ts. Used here as the
// canonical instance of cross-artifact authority binding so FU-T12's
...
function loadSweepWorkflow(): Workflow {
...
describe('cross-artifact validator: sweep.batch.items[].candidate_id ⊆ sweep.queue.to_execute', () => {
  let workflow: Workflow;
  let queueRel: string;
  let runRootBase: string;

  beforeEach(() => {
    workflow = loadSweepWorkflow();
    queueRel = artifactPathForSchemaInWorkflow(workflow, 'sweep.queue@v1');
    runRootBase = mkdtempSync(join(tmpdir(), 'circuit-next-cross-artifact-'));
  });
  ...
  it('agrees with the predicate over 200 deterministic samples with anti-vacuity floors per branch', () => {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      const runRoot = join(runRootBase, `iter-${i}`);
      mkdirSync(join(runRoot, dirname(queueRel)), { recursive: true });
      writeFileSync(join(runRoot, queueRel), sample.queueBody);
  ...
  it('returns fail when the queue file is missing', () => {
    const runRoot = join(runRootBase, 'no-queue');
    ...
    const result = runCrossArtifactValidator('sweep.batch@v1', workflow, runRoot, batchBody);
