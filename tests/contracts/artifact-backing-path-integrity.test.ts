...
import {
  ...
  RUNTIME_SAFETY_FLOOR_ARC_CEREMONY_SLICE,
...
describe('ARC_CLOSE_GATES + checkArcCloseCompositionReviewPresence (Slice 44 generalized gate)', () => {
  ...
  it('ARC_CLOSE_GATES contains all eleven arcs with matching ceremony_slice constants', () => {
    ...
    // as slice-55 + slice-62 + slice-68; numeric ceremony_slice: 75):
    // length bumped 6 -> 7 with the new runtime-safety-floor entry.
    ...
    // gate shape; numeric ceremony_slice: 92): length bumped 9 -> 10 with
    // the new p2-10-artifact-schema-tranche entry.
    // Slice 126 (Build Workflow Parity arc-close ceremony — same gate shape;
    // numeric ceremony_slice: 126): length bumped 10 -> 11 with the new
    // build-workflow-parity entry.
    expect(ARC_CLOSE_GATES).toHaveLength(11);
    ...
    const p210Arc = ARC_CLOSE_GATES.find((g) => g.arc_id === 'p2-10-artifact-schema-tranche');
    const buildArc = ARC_CLOSE_GATES.find((g) => g.arc_id === 'build-workflow-parity');
    ...
    expect(p210Arc).toBeDefined();
    expect(buildArc).toBeDefined();
    ...
    expect(runtimeSafetyArc?.ceremony_slice).toBe(RUNTIME_SAFETY_FLOOR_ARC_CEREMONY_SLICE);
    expect(typeof runtimeSafetyArc?.ceremony_slice).toBe('number');
    expect(runtimeSafetyArc?.plan_path).toBe('specs/plans/runtime-safety-floor.md');
    expect(
      runtimeSafetyArc?.review_file_regex.test(
        'arc-runtime-safety-floor-composition-review-claude.md',
      ),
    ).toBe(true);
    expect(
      runtimeSafetyArc?.review_file_regex.test(
        'arc-runtime-safety-floor-composition-review-codex.md',
      ),
    ).toBe(true);
    expect(runtimeSafetyArc?.review_file_regex.test('arc-slice-69-codex.md')).toBe(false);
    expect(runtimeSafetyArc?.review_file_regex.test('arc-slice-73-codex.md')).toBe(false);
    expect(p29Arc?.ceremony_slice).toBe(P2_9_SECOND_WORKFLOW_ARC_CEREMONY_SLICE);
    expect(typeof p29Arc?.ceremony_slice).toBe('number');
    expect(p29Arc?.plan_path).toBe('specs/plans/p2-9-second-workflow.md');
    expect(
      p29Arc?.review_file_regex.test('arc-p2-9-second-workflow-composition-review-claude.md'),
    ).toBe(true);
    expect(
      p29Arc?.review_file_regex.test('arc-p2-9-second-workflow-composition-review-codex.md'),
    ).toBe(true);
    ...
    expect(p210Arc?.ceremony_slice).toBe(P2_10_ARTIFACT_SCHEMA_ARC_CEREMONY_SLICE);
    expect(typeof p210Arc?.ceremony_slice).toBe('number');
    expect(p210Arc?.plan_path).toBe('specs/plans/phase-2-implementation.md');
    expect(
      p210Arc?.review_file_regex.test('arc-p2-10-artifact-schema-composition-review-claude.md'),
    ).toBe(true);
    expect(
      p210Arc?.review_file_regex.test('arc-p2-10-artifact-schema-composition-review-codex.md'),
    ).toBe(true);
    expect(p210Arc?.review_file_regex.test('arc-slice-89-codex.md')).toBe(false);
    expect(p210Arc?.review_file_regex.test('arc-slice-91-codex.md')).toBe(false);
    expect(buildArc?.ceremony_slice).toBe(BUILD_WORKFLOW_PARITY_ARC_CEREMONY_SLICE);
    expect(typeof buildArc?.ceremony_slice).toBe('number');
    expect(buildArc?.plan_path).toBe('specs/plans/build-workflow-parity.md');
    expect(
      buildArc?.review_file_regex.test('arc-build-workflow-parity-composition-review-claude.md'),
    ).toBe(true);
    expect(
      buildArc?.review_file_regex.test('arc-build-workflow-parity-composition-review-codex.md'),
    ).toBe(true);
    expect(buildArc?.review_file_regex.test('arc-slice-124-codex.md')).toBe(false);
    expect(buildArc?.review_file_regex.test('arc-slice-125-codex.md')).toBe(false);
  });
