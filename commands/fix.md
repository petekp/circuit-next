...
7. **If `outcome === "complete"`, read the Fix final report.** Surface
   ...
   `artifacts/fix-result.json` report. Surface its review result fields;
   to summarize the change and verification evidence, follow its
   `artifact_pointers` entries (in prose: evidence links — for example
...
- `src/workflows/fix/contract.md` (Fix report contract)
