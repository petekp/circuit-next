...
// named workflow fixture at `.claude-plugin/skills/<workflow-name>/circuit.json`,
// parses it through the production `Workflow` schema, calls
// `validateWorkflowKindPolicy`, composes the runtime boundary via the runner,
// and prints the <run-root> path on success.
//
...
export async function main(argv: readonly string[], options: CliMainOptions = {}): Promise<number> {
  ...
  if (
    ...
    const outcome = await resumeWorkflowCheckpoint({
      runRoot,
  ...
  const outcome = await runWorkflow(invocation);
  const resultPath =
