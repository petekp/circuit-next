...
export function writeManifestSnapshot(
  ...
  writeFileSync(manifestSnapshotPath(runFolder), `${JSON.stringify(snap, null, 2)}\n`);
  return snap;
}

export function readManifestSnapshot(runFolder: string): ManifestSnapshot {
...
export function verifyManifestSnapshotBytes(runFolder: string): ManifestSnapshot {
