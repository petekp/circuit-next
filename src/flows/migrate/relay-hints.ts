...
export const migrateInventoryShapeHint: SchemaShapeHint = {
  ...
  instruction: [
    ...
    'Each items[].id must be unique. Each batches[].item_ids[] must reference an items[].id (no orphans). The items array must contain at least one entry; if the walk finds nothing, investicheck further before responding rather than emitting an empty inventory. The batches array must contain at least one entry covering at least one item.',
