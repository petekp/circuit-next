...
// `path` is repository-relative; `category` lets the schematic narrate
...
// batches and relay them in parallel branches.
...
// parent check.pass admits — derived inside the child by
// deriveTerminalVerdict from its own review relay.
...
// schematic's verify step can read against a migrate-namespaced contract
// without forcing every flow to re-author the verification shape.
...
export type MigrateResultReportId = z.infer<typeof MigrateResultReportId>;

export const MigrateResultReportPointer = z
  .object({
...
export type MigrateResultReportPointer = z.infer<typeof MigrateResultReportPointer>;
...
export const MigrateResult = z
  .object({
    ...
    evidence_links: z.array(MigrateResultReportPointer).length(6),
  })
  .strict()
  .superRefine((result, ctx) => {
    const seen = new Set<MigrateResultReportId>();
