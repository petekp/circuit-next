...
**Last updated:** 2026-04-20 (Slice 24: HIGH #9 fold-in landed — grandfathered-review allowlist tightening. `GRANDFATHERED_CONTRACT_ALLOWLIST = {step.md, workflow.md}` pinned in `tests/contracts/cross-model-challenger.test.ts`; contracts outside the allowlist now fail the test if they carry `codex_adversarial_review_grandfathered`. Three new required fields on every grandfathered entry — `grandfathered_source_ref`, `grandfathered_scope`, `expires_on_contract_change: true` — added to step.md + workflow.md frontmatter (existing rationale prose stays). 484 tests (+2 new Slice 24 assertions), audit 10/0/0. Pre-Slice-24 fixup at `0103992` rewrote `scripts/audit.mjs` `countTests` to walk all `tests/**/*.test.*` files (was: only schema-parity.test.ts with static regex); static floor at HEAD is now 433 across 8 files — ratchet uses same method at both refs, runtime-total 484 stays the narrative-facing number.)
...
introduced the plane dimension (ADR-0004) on the authority graph;
