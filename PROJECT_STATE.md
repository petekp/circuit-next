...
**Last updated:** 2026-04-19 (autonomy arc slice 5: specs/contracts/selection.md v0.1)
**Phase:** 1 — Contract authorship **in progress**. Phase 0 Evidence Loop closed. Four Phase 1 contracts landed: `step.md` (MED #7 closed), `phase.md` (MED #11 closed + Codex objections folded in — v0.1 MED #7 now closed by selection.md SEL-I9), `run.md` (HIGH #1/#2 + MEDs #3-7 + LOWs #8-10 folded in), `selection.md` (Codex REJECT → ACCEPT: 6 HIGH + 4/5 MED + 1 LOW folded in; MED #9 scoped to v0.2). Bootstrap discipline (ADR-0002) codified; `.circuit/` gitignored; drift-visibility audit (`npm run audit`) enforces the citation rule + lane/framing discipline across every commit.
...
- Remaining contract stubs under `specs/contracts/`:
  - `adapter.md`, `continuity.md`, `skill.md`
...
### Closed this session (Phase 1, first contract slice + autonomy arc slices 1-5)
...
1. **Next contract: `adapter.md` or `continuity.md`?** `adapter.md` ratifies the `agent` vs `codex-isolated` naming question and the `dispatch.adapters` registry — load-bearing for the dispatch path. `continuity.md` governs cross-session handoff records (invoked by every resume). `skill.md` is the last of the four; less densely constrained. Recommended: `adapter.md` next — the dispatch layer has the most open design questions and is the most commonly exercised surface once implementation starts.
...
3. **Phase 2 property harness location.** Deferred property ids now span four contracts: run (4 props), phase (4 props), selection (6 props — including `resolved_matches_applied_composition`, `config_layer_precompose_is_right_biased`, `overlapping_phase_composition_well_defined`, `resolved_skills_are_unique_and_order_is_documented`). All need `tests/properties/visible/` per CLAUDE.md §Where things live. Not blocking Phase 1 authorship, but Phase 2 setup cost grows with each slice.
4. **Scalar tombstone semantics** (selection.md v0.1 Codex MED #9 deferred to v0.2). Should `SelectionOverride` allow explicit reset of `model`/`effort`/`rigor` to adapter-default? Skills have `{mode: 'replace', skills: []}` as the clear op; scalars have no analog. Adding a tombstone sentinel is a material design change and should be evidence-driven.
