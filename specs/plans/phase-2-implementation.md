...
| P2-5 | P2-MODEL-EFFORT landed | active — red | `src/runtime/selection-resolver.ts` + `tests/contracts/workflow-model-effort.test.ts` landed the runtime resolver/evidence seam at Slice 85. Still pending for satisfied status: product-path config discovery/loading into `selectionConfigLayers` and adapter-owned model/effort handling. Provider enum parse guard rejects unknown providers while model ids remain open strings per SEL-I4. Slice spec incorporated by reference from `specs/plans/phase-1-close-revised.md §Slice P2-MODEL-EFFORT`. |
...
- **P2-MODEL-EFFORT — explicit model + effort assignment (runtime
  resolver landed at Slice 85; product-path close still pending).**
  ...
  invocation layers. The live CLI still does not discover user-global or
  project config, and the built-in adapters do not yet validate or honor
  model/effort. The stale "unknown model id parse failure" wording is
