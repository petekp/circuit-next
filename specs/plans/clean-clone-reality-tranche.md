...
**Acceptance evidence:** The runner's dispatch branch schema-parses adapter output against `writes.artifact.schema` BEFORE passing the `artifact` slot into `materializeDispatch`. The artifact write requires BOTH (a) verdict gate pass per Slice 53 AND (b) schema parse success per Slice 54 — failure on either path leaves `writes.artifact.path` absent on disk. Parse failure emits `gate.evaluated` with `outcome: 'fail'` and a reason naming the schema parse error (mirroring the Slice 53 reject-on-bad-verdict shape so the failure-path event surface is uniform — NO separate `dispatch.failed` event type is introduced), then `step.aborted` with the same reason, then `run.closed outcome=aborted` with the same reason; `result.json` mirrors the close reason via RESULT-I4. New tests: `tests/runner/materializer-schema-parse.test.ts` covers (a) valid payload round-trip → artifact written + outcome=complete, (b) invalid payload → no artifact write + outcome=aborted with parse reason, (c) schema-missing fallback behavior (fail-closed — operator decision at implementation, but rejected is the preferred default per contract MUST), (d) interaction with Slice 53: gate-fail on bad verdict still skips artifact write even when the body would otherwise be schema-valid (regression for the Slice 53 HIGH 2 fold-in).
**Alternate framing:** (a) Schema-parse inside `materializeDispatch` itself. **Accepted as the implementation location at plan authoring; reversed at landing to runner-layer parse** because the runner already owns gate-evaluation in Slice 53 and routes evaluation failures into `gate.evaluated outcome=fail`; putting both checks at the same layer keeps the failure-path event surface uniform without duplicating schema logic across layers, AND materializeDispatch's direct callers in `tests/runner/{agent,codex}-dispatch-roundtrip.test.ts` use schema names (`agent.smoke@v1`, `codex.smoke@v1`) that are deliberately NOT in the registry — those tests bypass the runner and must continue to pass. (b) Extract a separate `validateDispatch` helper. **Accepted as the landed shape:** `src/runtime/artifact-schemas.ts::parseArtifact` is that helper; it lives outside both runner and materializer so either caller can consume it.
...
**Scope:**
1. New module `src/runtime/artifact-schemas.ts` — artifact-schema registry + `parseArtifact` helper. Registered schemas at v0.3 carry the minimal `{ verdict: z.string().min(1) }.passthrough()` shape; fail-closed on unknown schema names.
2. Amend `src/runtime/runner.ts` dispatch branch to call `parseArtifact` after Slice 53's `evaluateDispatchGate` admits a verdict, coercing the evaluation to `kind: 'fail'` on parse failure while preserving the observed verdict so `dispatch.completed.verdict` reflects what the adapter said.
3. Amend `src/runtime/adapters/dispatch-materializer.ts` header comment to strike the "v0 no schema parsing" claim; the function body is unchanged (by the time `writes.artifact` reaches the materializer, the caller has already decided the artifact is safe to write).
4. New test file `tests/runner/materializer-schema-parse.test.ts` — 4 cases exercising through `runDogfood` end-to-end.
5. Amend `specs/contracts/explore.md` with a new §Dispatch artifact schema-parse subsection + narrow the pre-Slice-53 dishonesty disclosure to "closed by Slice 53 for verdict admissibility; artifact-side closed by Slice 54."
6. Codex HIGH 1 fold-in: add `src/runtime/artifact-schemas.ts` to `AGENT_ADAPTER_SOURCE_PATHS` + `CODEX_ADAPTER_SOURCE_PATHS` in `scripts/audit.mjs` + the mirrored inline lists in `tests/runner/explore-e2e-parity.test.ts` + `tests/runner/codex-dispatch-roundtrip.test.ts` so a future registry change correctly trips AGENT_SMOKE / CODEX_SMOKE fingerprint drift.
7. Verify CC#P2-1 placeholder flow still completes under new validation (placeholder body is schema-valid by construction per Slice 44 amendment; close-step is synthesis not dispatch, so Slice 54's schema-parse path doesn't touch it).
**Ratchet:** Contract-test count advances. Audit-coverage may advance.
**Codex challenger:** REQUIRED.
...
4. Runner schema-parses adapter result_body against `writes.artifact.schema` before passing the artifact slot into `materializeDispatch`; failure emits `gate.evaluated outcome=fail` → `step.aborted` → `run.closed outcome=aborted` (same event-surface as the Slice 53 verdict-rejection path — no separate `dispatch.failed` event type).
