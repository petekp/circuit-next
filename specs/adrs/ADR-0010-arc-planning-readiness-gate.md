...
| 16 | `plan-lint.untracked-plan-cannot-claim-post-draft-status` | static-anchor | Untracked file with status beyond `evidence-draft`. The rule references `UNTRACKED_PERMITTED_STATUSES` = {`evidence-draft`}, a **hand-maintained singleton subset of `AUTHORING_STATUSES`** (Slice-66a Codex MED-1 fold-in: coupling to `AUTHORING_STATUSES` is editorial, not mechanical — editors must keep both sets consistent when the lifecycle evolves). `evidence-draft` is the only authoring status that permits untracked state. `challenger-pending` (the AUTHORING_STATUSES post-draft state) and every `COMMITTED_STATUSES` value require git-tracked state. |
